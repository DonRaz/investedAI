...
type SliderInfoKeys = 'marketReturn' | 'interestRate' | 'currentValue' | 'targetCash' | 'years' | 'taxRate' | 'profitPercentage';
...
const PortfolioTaxCalculator = () => {
  ...
  const sliderInfo = {
    marketReturn: {
      title: "Market Return Rate",
      description: "The expected annual percentage growth of your investment portfolio. Historical average for the S&P 500 is around 10%. Higher returns mean your portfolio grows faster, potentially offsetting interest costs in the loan strategy."
    },
    interestRate: {
      title: "Loan Interest Rate",
      description: "The annual interest rate charged on your portfolio-backed loan. These rates are typically lower than unsecured loans because your portfolio serves as collateral. Lower rates make the loan strategy more attractive compared to selling assets."
    },
    currentValue: {
      title: "Portfolio Value",
      description: "The current total market value of your investment portfolio. This is the combined value of all your investments before any taxes or fees."
    },
    targetCash: {
      title: "Target Cash",
      description: "The amount of money you need to access from your portfolio. This is the cash you're trying to obtain either by selling assets or by taking a loan against your portfolio."
    },
    years: {
      title: "Investment Period",
      description: "The number of years you plan to maintain your investment strategy before potentially selling or paying back loans. Longer periods typically favor the loan strategy as it gives your investments more time to compound."
    },
    taxRate: {
      title: "Tax Rate",
      description: "The percentage of your investment profits that will be paid as taxes when you sell. This includes federal, state, and local capital gains taxes. Higher tax rates make the loan strategy more attractive as it defers these taxes."
    },
    profitPercentage: {
      title: "Portfolio's Profit Percentage",
      description: "The percentage of your portfolio that consists of profits (capital gains). You only pay capital gains taxes on this portion. For example, if you invested $25,000 and now have $100,000, your profit percentage would be 75%."
    }
  };
  ...
  const calculateValues = () => {
    ...
    let totalLoanNeeded, yearlyInterest, monthlyInterest, accumulatedInterest = 0;
  ...
  const ClickableValue: React.FC<ClickableValueProps> = ({ value, refName, formatter = (v: number) => v.toString() }) => (
    <span 
  ...
  return (
    <div className={`font-sans bg-gradient-to-br from-blue-50 via-gray-50 to-indigo-50 dark:from-blue-950 dark:via-gray-950 dark:to-indigo-950 p-8 min-h-screen flex flex-col justify-center items-center ${direction() === 'rtl' ? 'rtl' : 'ltr'}`}>
        ...
        <CardContent className="space-y-8 p-8 relative z-10">
          ...
          <div className="p-6 backdrop-blur-md bg-gradient-to-r from-blue-50/90 to-blue-100/90 dark:from-blue-950/60 dark:to-blue-900/60 rounded-2xl border border-blue-200/50 dark:border-blue-800/30 shadow-lg">
            <p className="text-sm leading-relaxed text-gray-800 dark:text-gray-200">
              {t.ifYouHave} <ClickableValue value={inputs.currentValue} refName="currentValue" formatter={v => '$' + v.toLocaleString()} />, 
              {t.with} <ClickableValue value={inputs.profitPercentage} refName="profitPercentage" formatter={v => v + '%'} /> {t.inProfits}
              {t.andTaxRate} <ClickableValue value={inputs.taxRate} refName="taxRate" formatter={v => v + '%'} /> {t.taxRateOn}
              {t.andYouNeed} <ClickableValue value={inputs.targetCash} refName="targetCash" formatter={v => '$' + v.toLocaleString()} /> {t.inCash}
            </p>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <Label className="text-sm font-normal">Market Return Rate (%)</Label>
                ...
                <div className="flex justify-between text-xs text-gray-500">
                  <span>1%</span>
                  <span className="font-medium text-gray-800 dark:text-gray-200">{inputs.marketReturn}%</span>
                  <span>20%</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <Label className="text-sm font-normal">Loan Interest Rate (%)</Label>
                ...
                <div className="flex justify-between text-xs text-gray-500">
                  <span>1%</span>
                  <span className="font-medium text-gray-800 dark:text-gray-200">{inputs.interestRate}%</span>
                  <span>10%</span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <Label className="text-sm font-normal">Portfolio's Profit Percentage (%)</Label>
            ...
            <div className="flex justify-between text-xs text-gray-500">
              <span>0%</span>
              <span className="font-medium text-gray-800 dark:text-gray-200">{inputs.profitPercentage}%</span>
              <span>100%</span>
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 10 }}>
                ...
                <XAxis 
                  ...
                  label={{ value: 'Years', position: 'insideBottom', offset: -5, fill: '#888', fontSize: 12 }}
                />
                <YAxis 
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="bg-gradient-to-br from-blue-50/90 to-indigo-50/90 dark:from-blue-950/60 dark:to-indigo-950/60 backdrop-blur-md rounded-2xl p-6 shadow-md border border-blue-100/50 dark:border-blue-800/20">
              <h3 className="font-medium text-md mb-4 text-blue-800 dark:text-blue-300">Loan Strategy Results</h3>
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Required Loan:</span>
                  <span className="font-medium">${Math.round(summary.totalLoanNeeded).toLocaleString()}</span>
                </div>
                {loanStrategy === 'monthly' && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600 dark:text-gray-400">Monthly Payment:</span>
                    <span className="font-medium">${Math.round(finalYear.monthlyPayment).toLocaleString()}</span>
                  </div>
                )}
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">{loanStrategy === 'upfront' ? 'Prepaid Interest' : 'Total Interest Paid'}:</span>
                  <span className="font-medium">${Math.round(finalYear.totalInterestPaid).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Final Portfolio Value:</span>
                ...
                <div className="flex justify-between bg-gradient-to-r from-blue-100/90 to-indigo-100/90 dark:from-blue-900/70 dark:to-indigo-900/70 backdrop-blur-sm px-0 py-3 rounded-lg mt-4 text-sm border border-blue-200/50 dark:border-blue-700/30">
                  <span className="font-medium text-blue-800 dark:text-blue-200 pl-3">Net Worth:</span>
                  <span className="font-medium text-blue-800 dark:text-blue-200 pr-3">${(finalYear.loanStrategy - summary.totalLoanNeeded).toLocaleString()}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-red-50/90 to-rose-50/90 dark:from-red-950/60 dark:to-rose-950/60 backdrop-blur-md rounded-2xl p-6 shadow-md border border-red-100/50 dark:border-red-800/20">
              <h3 className="font-medium text-md mb-4 text-red-800 dark:text-red-300">Sell Strategy Results</h3>
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Amount to Sell:</span>
                  <span className="font-medium">${Math.round(summary.amountToSell).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Tax Paid:</span>
                  <span className="font-medium">${Math.round(summary.taxPaid).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Remaining Portfolio:</span>
                  <span className="font-medium">${Math.round(inputs.currentValue - summary.amountToSell).toLocaleString()}</span>
                </div>
                <div className="flex justify-between bg-gradient-to-r from-red-100/90 to-rose-100/90 dark:from-red-900/70 dark:to-rose-900/70 backdrop-blur-sm px-0 py-3 rounded-lg mt-4 text-sm border border-red-200/50 dark:border-red-700/30">
                  <span className="font-medium text-red-800 dark:text-red-200 pl-3">Final Portfolio Value:</span>
                  <span className="font-medium text-red-800 dark:text-red-200 pr-3">${finalYear.sellStrategy.toLocaleString()}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <span className="font-medium text-sm text-gray-600 dark:text-gray-400">{showDetails ? 'Hide Details' : 'Show Details'}</span>
          </Button>
          ...
          {showDetails && (
            <div className="mt-6 space-y-8 bg-gradient-to-br from-gray-50/90 via-white/80 to-gray-50/90 dark:from-gray-900/70 dark:via-gray-800/60 dark:to-gray-900/70 backdrop-blur-md rounded-2xl p-8 border border-white/50 dark:border-gray-700/30 shadow-lg">
              ...
              <div>
                <h4 className="font-medium text-md mb-4 bg-gradient-to-r from-gray-800 to-gray-600 dark:from-gray-200 dark:to-gray-400 bg-clip-text text-transparent">Strategy Comparison</h4>
                <div className="p-6 bg-gradient-to-b from-gray-50/80 to-gray-100/80 dark:from-gray-900/50 dark:to-gray-800/50 backdrop-blur-md rounded-2xl border border-gray-200/50 dark:border-gray-700/30 shadow-lg mb-6">
                  <div className="grid grid-cols-1 gap-y-6 text-sm">
                    <div className="flex items-start">
                      ...
                      <div>
                        <div className="font-medium">Loan Strategy</div>
                        <div className="text-xs text-gray-500 mt-1">
                          Keep investments growing while accessing cash without triggering capital gains
                        </div>
                      </div>
                    </div>
                    <div className="flex items-start">
                      ...
                      <div>
                        <div className="font-medium">Sell Strategy</div>
                        <div className="text-xs text-gray-500 mt-1">
                          Realize gains, pay taxes, and reduce your market exposure
                        </div>
                      </div>
                    </div>
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500 mt-1">
                          Portfolio value minus outstanding loan balance
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div>
                <h4 className="font-medium text-md mb-4 bg-gradient-to-r from-gray-800 to-gray-600 dark:from-gray-200 dark:to-gray-400 bg-clip-text text-transparent">Loan Payment Strategy</h4>
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  >
                    <span className={`font-medium mb-2 ${loanStrategy === 'upfront' ? 'text-blue-800 dark:text-blue-200' : ''}`}>
                      Upfront Interest
                    </span>
                    <span className="text-xs text-gray-500 max-w-[200px]">
                      Borrow extra initially to cover future interest
                    </span>
                  </Button>
                  <Button
                  ...
                  >
                    <span className={`font-medium mb-2 ${loanStrategy === 'monthly' ? 'text-blue-800 dark:text-blue-200' : ''}`}>
                      Monthly Payments
                    </span>
                    <span className="text-xs text-gray-500 max-w-[200px]">
                      Pay interest monthly from portfolio returns
                    </span>
                  </Button>
                </div>
              </div>
              ...
              <div>
                <h4 className="font-medium text-md mb-4 bg-gradient-to-r from-gray-800 to-gray-600 dark:from-gray-200 dark:to-gray-400 bg-clip-text text-transparent">Parameters</h4>
                <div className="space-y-6 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-xl p-6 shadow-md border border-white/50 dark:border-gray-700/30">
                  <div className="grid md:grid-cols-2 gap-8">
                    <div ref={portfolioValueRef} className={`space-y-4 ${animatingRef === 'currentValue' ? 'animate-pulse' : ''}`}>
                      <div className="flex items-center gap-2">
                        <Label className="text-sm font-normal">Portfolio Value</Label>
                        <HelpButton sliderKey="currentValue" />
                        <span className="ml-auto font-medium">${inputs.currentValue.toLocaleString()}</span>
                      </div>
                      <Slider
                        value={[inputs.currentValue]}
                        onValueChange={(value) => setInputs({ ...inputs, currentValue: value[0] })}
                        min={50000}
                        max={500000}
                        step={10000}
                        className="py-2"
                    ...
                    <div ref={targetCashRef} className={`space-y-4 ${animatingRef === 'targetCash' ? 'animate-pulse' : ''}`}>
                      <div className="flex items-center gap-2">
                        <Label className="text-sm font-normal">Target Cash</Label>
                        <HelpButton sliderKey="targetCash" />
                        <span className="ml-auto font-medium">${inputs.targetCash.toLocaleString()}</span>
                      </div>
                      <Slider
                        value={[inputs.targetCash]}
                        onValueChange={(value) => setInputs({ ...inputs, targetCash: value[0] })}
                        min={10000}
                        max={300000}
                        step={10000}
                        className="py-2"
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      <Label className="text-sm font-normal">Investment Period</Label>
                      <HelpButton sliderKey="years" />
                      <span className="ml-auto font-medium">{inputs.years} years</span>
                    </div>
                    <Slider
                      value={[inputs.years]}
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    <div ref={taxRateRef} className={`space-y-4 ${animatingRef === 'taxRate' ? 'animate-pulse' : ''}`}>
                      <div className="flex items-center gap-2">
              ...
              <div>
                <h4 className="font-medium text-md mb-4 bg-gradient-to-r from-gray-800 to-gray-600 dark:from-gray-200 dark:to-gray-400 bg-clip-text text-transparent">Formulas</h4>
                <div className="grid gap-3">
                  {Object.entries(formulas).map(([key, formula]) => (
                    <div key={key} className="p-4 bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-lg shadow-sm border border-white/50 dark:border-gray-700/30">
                      <p className="font-mono text-xs text-gray-600 dark:text-gray-400">{formula}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="w-full text-center pb-4">
        <p className="bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-400 dark:to-indigo-400 bg-clip-text text-transparent font-medium text-xs">
          Portfolio Tax Optimizer • Made with 💙
        </p>
      </div>
